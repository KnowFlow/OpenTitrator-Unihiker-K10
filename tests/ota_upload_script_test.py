...
class OtaUploadTest(unittest.TestCase):
    def test_cli_refuses_missing_password(self):
    ...
    def test_upload_sends_password_header_without_printing_it(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(OTA.urllib.request, "urlopen", return_value=response) as open_url:
                with redirect_stdout(output):
                    self.assertTrue(OTA.upload("192.0.2.1", str(firmware), "secret-password"))
        request = open_url.call_args.args[0]
        self.assertEqual(request.get_header("X-ota-password"), "secret-password")
    ...
    def test_http_error_does_not_print_token(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            error = urllib.error.HTTPError(
                "http://192.0.2.1/ota", 401, "Unauthorized secret-password", {}, None
            )
            with mock.patch.object(OTA.urllib.request, "urlopen", side_effect=error):
                with redirect_stdout(output):
                    self.assertFalse(OTA.upload("192.0.2.1", str(firmware), "secret-password"))
        self.assertNotIn("secret-password", output.getvalue())
