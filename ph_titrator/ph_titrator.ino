...
String htmlPage() {
  ...
  page += F("<section id='authPanel' class='card full'><div id='loginPanel'><h2>Controller login</h2><form id='loginForm' class='row'><label>Password<input name='password' type='password' autocomplete='current-password' required></label><button class='primary' type='submit'>Login</button></form></div><div id='sessionPanel' style='display:none'><span class='ok'>Authenticated</span> <button id='logoutButton' type='button'>Logout</button></div><details><summary>Factory recovery</summary><form id='recoveryForm' class='row'><label>Factory password<input name='factory_password' type='password' autocomplete='off' required></label><label>New password<input name='new_password' type='password' autocomplete='new-password' required></label><label>Confirm password<input name='confirm_password' type='password' autocomplete='new-password' required></label><button class='danger' type='submit'>Recover</button></form></details><p id='authMessage' class='tiny'></p></section>");
  ...
  page += F("if(sessionToken())showSession();else showLogin();document.querySelectorAll('.action').forEach(function(b){b.onclick=async function(){try{await apiPost('/action',{cmd:b.dataset.cmd},b.dataset.cmd==='panic');authMessage('Done');poll()}catch(e){authMessage(e.message)}}});document.querySelectorAll('.mutationForm').forEach(function(f){f.addEventListener('submit',async function(e){e.preventDefault();var data=Object.fromEntries(new FormData(f).entries());if(f.id==='manualForm'){data.cmd=e.submitter&&e.submitter.name?e.submitter.value:data.cmd;data.ajax='1'}try{await apiPost(f.action.endsWith('/action')?'/action':'/set',data,false);authMessage('Saved');poll()}catch(err){authMessage(err.message)}})});");
  page += F("document.getElementById('loginForm').addEventListener('submit',async function(e){e.preventDefault();try{var d=await apiPost('/login',Object.fromEntries(new FormData(e.currentTarget).entries()),true);sessionStorage.setItem('k10_session',d.token);e.currentTarget.reset();showSession();authMessage('Logged in')}catch(err){authMessage(err.message)}});document.getElementById('logoutButton').onclick=async function(){try{await apiPost('/logout',{},false)}catch(e){}sessionStorage.removeItem('k10_session');showLogin();authMessage('Logged out')};");
  page += F("document.getElementById('recoveryForm').addEventListener('submit',async function(e){e.preventDefault();var fd=new FormData(e.currentTarget),np=String(fd.get('new_password')||''),cp=String(fd.get('confirm_password')||''),bytes=new TextEncoder().encode(np).length;if(np!==cp){authMessage('New passwords do not match.');return}if(bytes<10||bytes>64){authMessage('New password must be 10 to 64 UTF-8 bytes.');return}try{await apiPost('/recover',{factory_password:fd.get('factory_password'),new_password:np},true);sessionStorage.removeItem('k10_session');e.currentTarget.reset();showLogin();authMessage('Recovery complete. Log in with the new password.')}catch(err){authMessage(err.message)}});");
  page += F("document.getElementById('otaForm').addEventListener('submit',async function(e){e.preventDefault();var fd=new FormData(e.currentTarget),headers={},token=sessionToken();if(token)headers['X-Session-Token']=token;try{var response=await fetch('/ota',{method:'POST',headers:headers,body:fd});var data=await response.json();if(response.status===401){sessionStorage.removeItem('k10_session');showLogin()}if(response.status===403)authMessage('Request is not permitted in the current state.');if(response.status===429)authMessage('Too many attempts. Try again later.');if(!response.ok)throw new Error(data.message||data.code||'Request failed');authMessage('Firmware uploaded')}catch(err){authMessage(err.message)}});");
