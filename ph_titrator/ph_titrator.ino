...
String htmlPage() {
  ...
  page += F("function drawCurve(){var c=document.getElementById('curveCanvas');if(!c)return;var r=c.getBoundingClientRect();if(r.width>0&&c.width!==Math.floor(r.width)){c.width=Math.floor(r.width);c.height=260}var g=c.getContext('2d'),w=c.width,h=c.height,l=58,t=18,ri=18,b=36;lastPlot=[];g.clearRect(0,0,w,h);g.fillStyle='#071014';g.fillRect(0,0,w,h);g.strokeStyle='#244c59';g.lineWidth=1;g.strokeRect(l,t,w-l-ri,h-t-b);text('curveInfo',curve.length+' points');text('eqpInfo',eqpText(currentEqp()));if(curve.length<2){drawDeriv();return}var xs=document.getElementById('chartX'),ys=document.getElementById('chartY');var xk=xs&&xs.value==='used'?'used_g':'elapsed_s';var ysel=ys?ys.value:'auto';var yk=ysel==='auto'?(curve[curve.length-1].endpoint==='mV'?'mv':'ph'):ysel;var pts=[];curve.forEach(function(p){if(xk==='used_g'&&pts.length&&Math.abs(p.used_g-pts[pts.length-1].used_g)<0.01){pts[pts.length-1]=p}else pts.push(p)});if(pts.length<2){text('curveInfo',curve.length+' points / waiting for '+(xk==='used_g'?'dose change':'more samples'));drawDeriv();return}var smooth=pts.map(function(p,i){var s=0,n=0;for(var j=Math.max(0,i-2);j<=Math.min(pts.length-1,i+2);j++){s+=pts[j][yk];n++}var q=Object.assign({},p);q[yk]=s/n;return q});var last=pts[pts.length-1],lastSmooth=smooth[smooth.length-1],tgt=last.target,tol=yk==='mv'?5:0.05;var minx=pts[0][xk],maxx=pts[0][xk],miny=pts[0][yk],maxy=miny;pts.concat(smooth).forEach(function(p){if(p[xk]<minx)minx=p[xk];if(p[xk]>maxx)maxx=p[xk];if(p[yk]<miny)miny=p[yk];if(p[yk]>maxy)maxy=p[yk]});if(tgt>0){miny=Math.min(miny,tgt-tol);maxy=Math.max(maxy,tgt+tol)}if(maxx===minx)maxx=minx+1;if(maxy===miny)maxy=miny+1;var ypad=(maxy-miny)*0.10;miny-=ypad;maxy+=ypad;var px=function(x){return l+(x-minx)/(maxx-minx)*(w-l-ri)};var py=function(y){return h-b-(y-miny)/(maxy-miny)*(h-t-b)};if(tgt>0){var ty1=py(tgt+tol),ty2=py(tgt-tol),ty=py(tgt);g.fillStyle='rgba(255,77,77,0.12)';g.fillRect(l,Math.min(ty1,ty2),w-l-ri,Math.abs(ty2-ty1));g.strokeStyle='#ff4d4d';g.lineWidth=1;g.setLineDash([6,4]);g.beginPath();g.moveTo(l,ty);g.lineTo(w-ri,ty);g.stroke();g.setLineDash([]);g.fillStyle='#ff6b6b';g.font='11px Verdana';g.fillText('target '+tgt.toFixed(yk==='ph'?2:0)+' ±'+tol.toFixed(yk==='ph'?2:0),w-ri-120,ty-5)}g.strokeStyle='rgba(103,240,154,0.28)';g.lineWidth=1;g.beginPath();pts.forEach(function(p,i){var x=px(p[xk]),y=py(p[yk]);lastPlot.push({x:x,y:y,p:p,yk:yk});if(i)g.lineTo(x,y);else g.moveTo(x,y)});g.stroke();g.strokeStyle='#67f09a';g.lineWidth=3;g.beginPath();smooth.forEach(function(p,i){var x=px(p[xk]),y=py(p[yk]);if(i)g.lineTo(x,y);else g.moveTo(x,y)});g.stroke();var lx=px(last[xk]),ly=py(lastSmooth[yk]);g.fillStyle='#eaf7ff';g.strokeStyle='#071014';g.lineWidth=2;g.beginPath();g.arc(lx,ly,5,0,6.283);g.fill();g.stroke();var err=last[yk]-tgt,tag=(yk==='ph'?last[yk].toFixed(2):last[yk].toFixed(0))+' '+yk+(tgt>0?' / Δ '+(err>=0?'+':'')+err.toFixed(yk==='ph'?2:0):'');g.fillStyle='rgba(7,16,20,0.86)';g.fillRect(Math.max(l,lx-154),Math.max(t,ly-28),150,22);g.fillStyle='#eaf7ff';g.font='12px Verdana';g.fillText(tag,Math.max(l+4,lx-150),Math.max(t+14,ly-13));var e=currentEqp();if(e){var ex=px(xk==='used_g'?e.used_g:e.elapsed_s),ey=py(yk==='mv'?e.mv:e.ph);g.strokeStyle='#ffd15c';g.fillStyle='#ffd15c';g.lineWidth=1;g.beginPath();g.moveTo(ex,t);g.lineTo(ex,h-b);g.stroke();g.beginPath();g.arc(ex,ey,5,0,6.283);g.fill()}var lastPump=false;var doseCount=0;curve.forEach(function(p){if(p.pump&&!lastPump&&p[xk]>=minx&&p[xk]<=maxx){var dx=px(p[xk]);g.strokeStyle='rgba(255,210,74,0.4)';g.lineWidth=1;g.setLineDash([2,6]);g.beginPath();g.moveTo(dx,t);g.lineTo(dx,h-b);g.stroke();g.setLineDash([]);if(++doseCount<=8){g.fillStyle='rgba(255,210,74,0.7)';g.font='9px Verdana';g.fillText((p.pulse_ms||0)+'ms',dx+2,t+10)}}lastPump=p.pump});g.fillStyle='#8db0bd';g.font='12px Verdana';g.fillText(xk==='used_g'?'used g':'time s',w-86,h-10);g.fillText(yk,l,13);g.fillText(miny.toFixed(yk==='ph'?2:0),8,h-b);g.fillText(maxy.toFixed(yk==='ph'?2:0),8,t+5);drawDeriv()}");
