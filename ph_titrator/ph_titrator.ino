...
String htmlPage() {
  ...
  page += F("var runRecord=null,recordDb=null;function recordCopy(v){return JSON.parse(JSON.stringify(v))}function readRecordMetadata(){return{sampleId:document.getElementById('recordSampleId').value,batchReference:document.getElementById('recordBatchReference').value,operator:document.getElementById('recordOperator').value,notes:document.getElementById('recordNotes').value}}function newRunRecord(){runRecord={schemaVersion:1,recordStatus:'draft',metadata:readRecordMetadata(),startedAt:null,finishedAt:null,methodSnapshot:null,deviceSnapshot:null,confirmed:null,final:null,points:[]};renderRunRecord()}function renderRunRecord(){text('recordStatus',runRecord?runRecord.recordStatus.charAt(0).toUpperCase()+runRecord.recordStatus.slice(1):'Draft');text('recordPoints',runRecord?runRecord.points.length+' points':'0 points')}function recordMethodSnapshot(d){return{method:d.method,methodLabel:d.method_label,endpoint:d.endpoint,targetPh:d.target_ph,targetMv:d.target_mv,maxGrams:d.max_g,controlBand:d.control_band,stableDelta:d.stable_delta,holdSeconds:d.hold_s,minSettleSeconds:d.min_settle_s,maxSettleSeconds:d.max_settle_s,titrant:d.titrant,titrantM:d.titrant_m,resultFormula:d.result_formula,blankGrams:d.blank_g,titrantDensity:d.titrant_density,sampleDensity:d.sample_density,manualFactor:d.manual_factor}}function recordDeviceSnapshot(d){return{network:d.network,ip:d.ip,ap_ip:d.ap_ip,sta_ip:d.sta_ip,sta_connected:d.sta_connected,ota:d.ota,ota_safety_lock:d.ota_safety_lock,status:d.status,titrantGps:d.titrant_gps,sampleGps:d.sample_gps,titrantRunUs:d.titrant_run_us,sampleRunUs:d.sample_run_us,titrantDosePercent:d.titrant_dose_pct,scaleFactor:d.scale_factor,phCalibrationStatus:d.ph_cal_status,phSlopePercent:d.ph_slope_percent,phOffset7Mv:d.ph_offset7_mv}}function recordPoint(){var p=recordCopy(curve[curve.length-1]);p.elapsed_s=runRecord.points.length?(new Date(p.ts)-new Date(runRecord.startedAt))/1000:0;return p}function recordFinal(d){return{state:d.state,status:d.status,resultValue:d.result_value,resultUnit:d.result_unit,usedGrams:d.used_g,endpointUsedGrams:d.endpoint_used_g,sampleDeliveredGrams:d.sample_delivered_g,eqpReached:d.eqp_reached,eqpUsedGrams:d.eqp_used_g,eqpSignal:d.eqp_signal,eqpSlope:d.eqp_slope}}function observeRunRecord(d){var active=['SAMPLE','WARMUP','RUN','DOSE','WAIT','PAUSE'].indexOf(d.state)>=0;if(active&&!runRecord)newRunRecord();if(active&&runRecord&&!runRecord.startedAt){runRecord.recordStatus='running';runRecord.startedAt=new Date().toISOString();runRecord.methodSnapshot=recordCopy(recordMethodSnapshot(d));runRecord.deviceSnapshot=recordCopy(recordDeviceSnapshot(d));runRecord.points=d.adc_ok&&curve.length?[recordPoint()]:[]}else if(runRecord&&runRecord.startedAt&&!runRecord.final&&d.adc_ok&&curve.length){runRecord.points.push(recordPoint())}if(runRecord&&runRecord.startedAt&&!runRecord.final){var completed=d.state==='DONE'&&(d.status==='Target reached'||d.status==='Equivalence point');var aborted=d.state==='ERROR'||d.status==='Emergency stop'||(!active&&d.state!=='DONE');if(completed||aborted){runRecord.recordStatus=completed?'completed':'aborted';runRecord.confirmed=completed;runRecord.finishedAt=new Date().toISOString();runRecord.final=recordCopy(recordFinal(d));saveFinalRecord(runRecord)}}renderRunRecord()}");
  page += F("function openRecordDb(){return new Promise(function(resolve,reject){if(recordDb)return resolve(recordDb);var request=indexedDB.open('k10-titration-records',1);request.onupgradeneeded=function(){if(!request.result.objectStoreNames.contains('runs'))request.result.createObjectStore('runs',{keyPath:'id'})};request.onsuccess=function(){recordDb=request.result;resolve(recordDb)};request.onerror=function(){reject(request.error)}})}function savedSummary(r){var m=r.metadata||{};return(r.finishedAt||r.savedAt||'').replace('T',' ').slice(0,19)+' / '+(m.sampleId||'unnamed')+' / '+r.recordStatus}function refreshSavedRecords(){openRecordDb().then(function(db){var request=db.transaction('runs').objectStore('runs').getAll();request.onsuccess=function(){var rows=request.result.sort(function(a,b){return String(b.savedAt).localeCompare(String(a.savedAt))}),select=document.getElementById('savedRecordSelect');select.innerHTML='<option value=&quot;&quot;>Select saved record</option>';rows.forEach(function(r){var option=document.createElement('option');option.value=r.id;option.textContent=savedSummary(r);select.appendChild(option)});text('savedRecordInfo',rows.length+' saved records in this browser.')}}).catch(function(){text('savedRecordInfo','Local record storage is unavailable.')})}function saveFinalRecord(record){if(!record||!record.final)return;openRecordDb().then(function(db){var copy=recordCopy(record);copy.id=copy.id||('run-'+Date.now()+'-'+Math.random().toString(16).slice(2));copy.savedAt=new Date().toISOString();record.id=copy.id;var tx=db.transaction('runs','readwrite'),store=tx.objectStore('runs');store.put(copy);store.getAll().onsuccess=function(e){e.target.result.sort(function(a,b){return String(b.savedAt).localeCompare(String(a.savedAt))}).slice(50).forEach(function(old){store.delete(old.id)})};tx.oncomplete=refreshSavedRecords;tx.onerror=function(){text('savedRecordInfo','Could not save this local record.')}}).catch(function(){text('savedRecordInfo','Local record storage is unavailable.')})}function loadSavedRecord(){var id=document.getElementById('savedRecordSelect').value;if(!id)return;openRecordDb().then(function(db){var request=db.transaction('runs').objectStore('runs').get(id);request.onsuccess=function(){if(!request.result)return;runRecord=recordCopy(request.result);curve=recordCopy(runRecord.points||[]);curveStart=curve.length?Date.now()-num(curve[curve.length-1].elapsed_s)*1000:0;eqpManual=null;renderRunRecord();drawCurve();text('savedRecordInfo','Loaded local record.')}})}function deleteSavedRecord(){var id=document.getElementById('savedRecordSelect').value;if(!id)return;openRecordDb().then(function(db){var tx=db.transaction('runs','readwrite');tx.objectStore('runs').delete(id);tx.oncomplete=refreshSavedRecords})}");
  ...
  page += F("function retainSavedRecords(records){return records.slice().sort(function(a,b){return String(b.savedAt).localeCompare(String(a.savedAt))}).slice(0,50)}");
