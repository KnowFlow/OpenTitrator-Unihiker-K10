...
4. 2 秒后，**滴定泵**运行 2 秒。
5. 静置 4 秒后，**样品泵**运行 2 秒。
6. 再静置 4 秒后，系统计算流量并保存。
...
2. **Dosing（加药中）** — 滴定泵按脉冲时长运行（30–600 ms）。
3. **Settling（静置中）** — 等待反应平衡（2–8 s）。
...
**结果显示**：
- `RESULT 0.00307M` 为计算出的样品浓度（mol/L）。
- 公式：`C_样品 = C_滴定剂 × m_滴定剂 / m_样品`
...
若斜率陡峭（|dpH/dt| > 0.08）→ 30 ms 脉冲，静置 8 s
否则若误差 > 1.0 → 600 ms 脉冲，静置 2 s
否则若误差 > 0.3 → 200 ms 脉冲，静置 3.5 s
否则 → 80 ms 脉冲，静置 5 s
```
