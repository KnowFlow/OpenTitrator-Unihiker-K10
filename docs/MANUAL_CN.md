...
- **电气安全**：K10 逻辑电路为 3.3 V，蠕动泵必须使用符合新品样机工程规格的外置稳压电源。**严禁**直接从 K10 板载电源驱动泵。
...
| 2 | DFRobot 未上线蠕动泵新品 | 待官方上线后补充 |
| 各 1 | pH 电极/前端、反应容器、K10 USB-C 电源和外置泵电源 | — |
...
- **滴定泵**信号线 → `P0`
- **样品泵**信号线 → `P1`
- 泵由**外置稳压电源**供电，不接 K10；官方规格发布前按新品样机工程规格选型。
...
首次配置时，请用设备私密标签上的唯一出厂密码登录并设置管理员密码；以后使用管理员密码登录。网页标题栏的语言选择器可在中文和英文之间切换完整界面，并把选择保存在当前浏览器。
...
如果样品已经在反应瓶中，可在网页 Run tab 使用 **Start existing sample（使用已有样品开始）**。此命令跳过 `SampleFilling`，按设置的样品量记录实验并直接进入 `FilterWarmup`。操作前必须确认设置的样品质量与实际样品一致。
...
- **Saved records** 会在实验完成或中止后自动保存在当前浏览器的 IndexedDB 中；运行中的记录仍只在内存中，避免高频写入。可从下拉列表加载、导出、打印或删除历史记录；浏览器最多保留最近 50 条。记录不会上传到 K10，清除浏览器站点数据会一并清除这些本地历史。
- **CSV** / **JSON** 将当前曲线数据下载到电脑，K10 不会把曲线写入 flash。
- CSV / JSON 导出会包含当前 EQP 候选点、信号值和最大斜率。
- 刷新网页会清空仍在运行的内存曲线；已结束并保存的历史记录仍会保留，除非清除浏览器站点数据。
...
CLI 脚本需要当前已认证会话的令牌；网页 **Firmware update（固件更新）** 表单会自动使用当前浏览器会话，日常操作优先使用网页。请勿把令牌写入命令历史、日志、截图或问题记录。PowerShell 等价命令为 `.\scripts\ota_upload.ps1 -Bin firmware.bin -Ip DEVICE_IP -Token SESSION_TOKEN`。
...
如果出厂恢复标签遗失，且无法通过网页恢复，维护人员可以在物理 USB 连接下使用
`unihiker-auth-recovery` 恢复镜像写入新的管理员凭据。该镜像只更新
`auth` Preferences 命名空间，不会清除泵校准、pH 校准、方法参数或 Wi-Fi 设置；它不初始化或驱动任何泵。

恢复镜像成功启动后会在 USB 串口输出 `AUTH_RECOVERY:OK`。必须立即刷回通常的
`unihiker` 固件，之后用新管理员密码登录。恢复构建使用的
`recovery_admin.generated.h` 是本机临时文件，必须保持忽略且在操作完成后删除；密码、盐值、派生哈希和会话令牌都不得写入 Git、串口日志或问题记录。
