...
- [x] 基于 M3 数据计算 `d(signal)/d(used_g)`。
- [x] 找最大斜率点作为候选 EQP。
- [x] 在曲线上标记 EQP。
- [x] 输出：
  - endpoint used grams
...
- [x] 网页显示结果和单位。
- [x] 导出数据时包含结果。
- [x] 增加滴定剂密度和样品密度参数，默认 `1.000 g/mL`，用于把称重结果显式换算为 mL。
- [x] EDTA 硬度方法从固定 mV 终点升级为基于 `d(mV)/d(used)` 的自动等当点停止。
- [ ] 通过真实硬度样品调试自动 EQP 的峰值阈值、回落比例和确认点数。
- [ ] 总硬度方法成熟后，再设计钙/镁两步法；pH 10 和 pH 12-13 条件控制先记录为后续工作，不急于增加第二支 pH 计。

验收：

- [x] EDTA 方法能输出 CaCO3 mg/L。
- [x] 手动方法能通过用户系数输出自定义结果。
- [x] 结果公式不会影响泵控制。
...
- [x] 空白值和辅助值管理：按 Method 保存 blank、手动浓度、密度和手动系数。
- [ ] 系列样品记录。
- [ ] 报告模板。
